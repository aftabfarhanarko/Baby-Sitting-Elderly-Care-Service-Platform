...
export const authOptions = {
  providers: [
    GoogleProvider({
      ...
      clientSecret: process.env.CLIENT_SECRIET || "",
    }),
    CredentialsProvider({
      name: "Credentials",
      credentials: {
      ...
      async authorize(credentials) {
        ...
        return {
          ...
          name: user.name,
          email: user.email,
          image: user.profileImage || user.photo || null,
        ...
        try {
          ...
          if (!existingUser) {
            await usersCollection.insertOne({
              name: user.name,
              email: user.email,
              profileImage: user.image, // Saved as profileImage to match register
              role: "user",
      ...
      if (user) {
        if (account?.provider === "google") {
          ...
          if (dbUser) {
            ...
            token.image = dbUser.profileImage || dbUser.photo;
          }
        } else {
          token.id = user.id;
